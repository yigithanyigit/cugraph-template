  ...
  bool renumber)
{
  ...
  if (d_wgt)
    *d_wgt = cugraph::utilities::device_gatherv(
      handle, raft::device_span<weight_t const>{d_wgt->data(), d_wgt->size()});
  if (d_edge_id)
    *d_edge_id = cugraph::utilities::device_gatherv(
  ...
  if (renumber_map) { vertices = cugraph::utilities::device_gatherv(handle, *renumber_map); }
  ...
  raft::device_span<value_t const> mg_values)
{
  ...
  if (mg_renumber_map) {
    if (mg_vertices) {
      ...
      mg_aggregate_vertices = cugraph::utilities::device_gatherv(
        handle, raft::device_span<vertex_t const>(local_vertices.data(), local_vertices.size()));
    } else {
      mg_aggregate_vertices = cugraph::utilities::device_gatherv(handle, *mg_renumber_map);
    }
  } else {
    if (mg_vertices) {
      mg_aggregate_vertices = cugraph::utilities::device_gatherv(handle, *mg_vertices);
    } else {
  ...
  auto mg_aggregate_values = cugraph::utilities::device_gatherv(handle, mg_values);
  ...
  if (handle.get_comms().get_rank() == 0) {
    ...
    std::tie(sg_vertices, sg_values) = cugraph::utilities::sort_by_key<vertex_t, value_t>(

...
#include "csv_file_utilities.hpp"
#include "matrix_market_file_utilities.hpp"
 ...
 public:
  ...
  void translate(raft::handle_t const& handle, rmm::device_uvector<vertex_t>& vertices) const
  {
    if (base_vertex_id_ > 0) {
      cugraph::utilities::translate_vertex_ids(handle, vertices, static_cast<vertex_t>(base_vertex_id_));
    }
  }
  ...
  {
    if (base_vertex_id_ > 0) {
      cugraph::utilities::translate_vertex_ids(handle, srcs, static_cast<vertex_t>(base_vertex_id_));
      cugraph::utilities::translate_vertex_ids(handle, dsts, static_cast<vertex_t>(base_vertex_id_));
    }
  }
...
class File_Usecase : public detail::TranslateGraph_Usecase {
 public:
  ...
  {
    /* UNNECESSARY
    //  for now, we don't need rapids dataset root dir
    if ((graph_file_path.length() > 0) && (graph_file_path[0] != '/')) {
      graph_file_full_path_ = cugraph::utilities::get_rapids_dataset_root_dir() + "/" + graph_file_path;
    } else {

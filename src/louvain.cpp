...
// Defaul managed memory is false
...
int main(int argc, char **argv) {
  ...
  if (argc < 5) {
    std::cerr << std::endl
              << "Usage: " << argv[0]
              << " <path to mtx file> <max level> <threshold> <resolution>"
              << std::endl
              << "Example: " << argv[0]
              << " ../../datasets/karate.mtx 10 0.5 1.0" << std::endl;
    exit(1);
  } else {
    ...
    if (argc == 6) {
      au.managed_memory = std::stoi(argv[5]);
    }
  }

  switch (au.managed_memory) {
  case 0:
    ...
    break;
  case 1:
    std::cout << "Using managed memory resource" << std::endl;
    static rmm::mr::managed_memory_resource mr{};
  ...
  auto [graph, edge_weights, d_renumber_map_labels] =
      cugraph::utilities::construct_graph<int64_t, int64_t, float, false,
                                          false>(handle, usecase, true, false);
  ...
  try {
    ...
    auto [num_levels, modularity] = cugraph::louvain(
        ...
        std::optional<std::reference_wrapper<raft::random::RngState>>{
            std::nullopt},
        graph.view(), edge_weight_view, clustering.data(), au.max_level,
        au.threshold, au.resolution);
